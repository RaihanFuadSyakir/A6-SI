...
@app.route('/get_past_posts/<username>', methods=['GET'])
...
@app.route('/get_past_comments/<post_pk>', methods=['GET'])
...
async def run_sync_user(username, progress_id):
    try:
        await scrapper.sync_user(username)
        # Update progress to 100 upon completion
        db['progress_collection'].update_one(
            {'_id': progress_id}, {'$set': {'progress': 100, 'is_done': True}})
    except Exception as e:
