...
export default function AnalyzeMetric({posts,setPosts,index}:PostSet) {
    const [isSync,setSync] = useState(false);
    function sync_post(e : any){
      setSync(true);
      axiosInstance.post(`/analyze_metrics/${posts[index].post_pk}`)
    ...
    return (
      <IconButton color="primary" aria-label="sync" className={`p-0 rounded-full hover:bg-sky-300`} onClick={sync_post}>
        {isSync ? (<CircularProgress/>):(<AddchartIcon/>)}
      </IconButton>
